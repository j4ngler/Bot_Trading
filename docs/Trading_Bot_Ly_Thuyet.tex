...
\begin{itemize}
    ...
    \begin{itemize}
        \item RSI > 70 hiểu như trạng thái ``đã có quá nhiều người mua'' $\rightarrow$ cầu suy yếu $\rightarrow$ rủi ro đảo chiều tăng.
        \item RSI < 30 hiểu như ``đã có quá nhiều người bán'' $\rightarrow$ áp lực cung suy yếu $\rightarrow$ dễ phục hồi.
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Chặn giao dịch nếu RSI > 75 (quá mua)
        \item Chặn nếu biến động quá cao: ATR/giá > 5\%
    \end{itemize}
    \item Khi RSI > 70 hoặc ATR/giá > 5\% $\rightarrow$ Bot \textbf{tự động từ chối} giao dịch
...
\begin{itemize}
    ...
    \begin{itemize}
        \item RSI cực cao (>75) $\rightarrow$ Nhiều người đã mua
        \item Biến động cao (ATR/giá > 5\%) $\rightarrow$ Thị trường không ổn định
    \end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item RSI thấp (20-30), giá ổn định
        \item Ít người quan tâm
        \item Bot: Có thể mua (cơ hội tốt)
    \end{itemize}
    ...
    \begin{itemize}
        \item Giá giảm mạnh
        \item RSI rất thấp (<25)
        \item Bot: Cân nhắc mua (cơ hội hồi phục)
    \end{itemize}
...
\begin{itemize}
    \item Nếu RSI > 75: ``RSI quá cao - Thị trường quá mua (Hưng phấn)'' $\rightarrow$ Không giao dịch
    \item Nếu RSI < 25: ``RSI quá thấp - Thị trường quá bán (Hoảng loạn)'' $\rightarrow$ Không giao dịch
\end{itemize}
...
\midrule
\textbf{FOMO} & Chặn giao dịch khi RSI > 75 hoặc ATR/giá > 5\% & RSI = 78 $\rightarrow$ Không giao dịch \\
\midrule
\textbf{Herding} & Nhận diện cực trị qua RSI + ATR & RSI cao + ATR cao $\rightarrow$ Bỏ qua \\
\midrule
...
\midrule
\textbf{Chu kỳ tâm lý} & Nhận diện vùng cực trị (RSI > 75 hoặc < 25) & RSI = 78 $\rightarrow$ ``Hưng phấn'' $\rightarrow$ Không giao dịch \\
\bottomrule
\end{longtable}
...
\begin{itemize}
    ...
    \item \textbf{Chế độ 2: Chạy LIÊN TỤC} - Tự động mỗi 15 phút
